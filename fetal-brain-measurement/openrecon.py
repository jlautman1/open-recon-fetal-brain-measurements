...
class FetalBrainI2IHandler:
    ...
    def process(self, input_image, output_image, metadata):
        ...
        try:
            ...
            # Handle different metadata types (dict vs ismrmrdHeader object)
            try:
                if hasattr(metadata, 'patientInformation') and metadata.patientInformation:
                    patient_id = getattr(metadata.patientInformation, 'patientID', '99999')
                else:
                    patient_id = '99999'
            except:
                patient_id = '99999'
            ...
            try:
                if hasattr(metadata, 'measurementInformation') and metadata.measurementInformation:
                    series_num = getattr(metadata.measurementInformation, 'seriesNumber', '1')
                else:
                    series_num = '1'
            except:
                series_num = '1'
            ...
            try:
                # Try to get resolution from metadata encoding
                if hasattr(metadata, 'encoding') and metadata.encoding:
                    encoding = metadata.encoding[0]
                    if hasattr(encoding, 'encodedSpace'):
                        fov = encoding.encodedSpace.fieldOfView_mm
                        matrix = encoding.encodedSpace.matrixSize
                        res_x = f"{fov.x / matrix.x:.3f}"
                        res_y = f"{fov.y / matrix.y:.3f}"
                        res_z = f"{fov.z / matrix.z:.1f}"
                    else:
                        raise AttributeError("No encodedSpace found")
                else:
